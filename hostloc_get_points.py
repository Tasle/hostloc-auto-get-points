...
def get_points(s: req_Session, number_c: int):
    if check_login_status(s, number_c):
        befpoints = print_current_points(s)  # 打印帐户当前积分
        time.sleep(5)
        ...
        for i in range(len(url_list)):
            ...
            try:
                ...
                print("链接访问异常：" + str(e))
            continue
        # print("共", i, "个用户空间链接访问成功")
        aftpoints = print_current_points(s)  # 再次打印帐户当前积分
        time.sleep(5)
        return befpoints, aftpoints, i, "第%d个账户登录成功" % number_c
    else:
        print("请检查你的帐户是否正确！")
        return befpoints, aftpoints, i, "第%d个账户登录失败" % number_c

    # 打印输出当前ip地址
def print_my_ip():
    # api_url = "https://api.ipify.org/"
    api_url = "https://www.athony.net/ip/"
    try:
        res = requests.get(url=api_url)
        res.raise_for_status()
        res.encoding = "utf-8"
        print("ip 地址：" + res.text)
    except Exception as e:
        print("获取ip 地址失败：" + str(e))
...
if __name__ == "__main__":
    ...
    if not username or not password:
        ...
        content += "未检测到用户名或密码，请检查环境变量是否设置正确！"
    elif len(user_list) != len(passwd_list):
        print("用户名与密码个数不匹配，请检查环境变量设置是否错漏！")
        content += "用户名与密码个数不匹配，请检查环境变量设置是否错漏！"
    else:
        print_my_ip()
        print("共检测到", len(user_list), "个帐户，开始获取积分")
        print("*" * 30)
        content += "获取ip 地址：" + requests.get(url="https://www.athony.net/ip/").text + "\n"
        content += "共检测到%d" % (len(user_list)) + "个帐户，开始获取积分\n"
        # 依次登录帐户获取积分，出现错误时不中断程序继续尝试下一个帐户

        for i in range(len(user_list)):
            try:
                s = login(user_list[i], passwd_list[i])
                rca = get_points(s, i + 1)
                print("*" * 30)
                content += (
                        "\n------第%d个账户------" % (i + 1) + "\n"
                        "------\n"
            ...
            continue
        url = 'https://service-3jhcbs0d-1301269162.sh.apigw.tencentcs.com/release/PushWC'
        response = requests.get(url, params={"type": "tc", "text": "HostLoc签到", "desp": content})
